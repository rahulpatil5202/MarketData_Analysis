...
```{r}
### data Processing
nse <- dbGetQuery(cn1,'select nse.trade_date, nse.symbol, nse.isin, industryclass.sector_name, industryclass.industry_name, nse.close  from nse
...
d7 <- nse[nse$trade_date %in% nse_days[7,1],]
...
trend_data <- d1
trend_data$d7 <- d7[match(trend_data$isin, d7$isin),'close']
...
trend_data <- trend_data[complete.cases(trend_data[7:12]),]

trend_data$d7_prct <- round(((trend_data$close - trend_data$d7)/trend_data$d7) * 100,2)
trend_data$d60_prct <- round(((trend_data$close - trend_data$d60)/trend_data$d60) * 100,2)
trend_data$d120_prct <- round(((trend_data$close - trend_data$d120)/trend_data$d120) * 100,2)
trend_data$d180_prct <- round(((trend_data$close - trend_data$d180)/trend_data$d180) * 100,2)
trend_data$d240_prct <- round(((trend_data$close - trend_data$d240)/trend_data$d240) * 100,2)
trend_data$d300_prct <- round(((trend_data$close - trend_data$d300)/trend_data$d300) * 100,2)
trend_data$d360_prct <- round(((trend_data$close - trend_data$d360)/trend_data$d360) * 100,2)

d7_top10 <- top_n(trend_data, 10, wt = trend_data$d7_prct)
...
row{}
----------------------
### Top 30 performers merged (ST & LT)


```{r}
d360_top30 <- top_n(trend_data, 30, wt = trend_data$d360_prct)
d60_top30 <- top_n(trend_data, 30, wt = trend_data$d60_prct)

recommended_stocks = d360_top30[d360_top30$symbol %in% d60_top30$symbol,]

datatable(recommended_stocks,
          rownames = T,
          filter = "top",
          options = list(pageLength = 100))
...
row{}
----------------------
### Heroes of the week (Best performer last 7 trades)
