...
ts.dt <- yes_bank[,1]
ts.df <- yes_bank[,-1]
std.dt <- seq.Date(from = as.Date("2005-07-12"), to = as.Date("2018-06-05"), by = 1)
empty.ts <- xts(,std.dt)
yes_ts <- xts(ts.df,ts.dt, frequency = 5)


yes_ts <- merge(empty.ts, yes_ts)
yes_ts <- na.locf(yes_ts)


## Creating new ts() object to decompose seasonality, trend and error
yes_ts1 <- as.ts(yes_ts)
yes_ts1 <- ts(yes_ts1, frequency = 12)
...
plot.zoo(yes_ts, plot.type = "multiple") #plot type adds no. of screens for each plot
